...
|---------------|----------------|
| **Professor** | Acesso **apenas às turmas em que está cadastrado** como professor. **Avaliação mensal** do aluno: conceito geral; evolução em escrita, leitura, matemática; atenção e comportamento; dificuldade percebida (se sim, deve justificar, informar se recente/recorrente e se já usou estratégias em sala). Ao salvar, o sistema recalcula o risco; aluno com conceito **insuficiente** em qualquer área entra no **painel de alertas** da coordenação. Pode elaborar **PEI guiado** (wizard em 9 etapas) quando a equipe multidisciplinar recomendar ou quando necessário; o sistema pré-preenche capacidades, necessidades e objetivos a partir da avaliação e sugestões. Além disso, pode emitir Alertas Críticos (Dossiê #OC-1) para situações de risco iminente ou proteção à criança ao radar prioritário da coordenação. |
...
| **PEI guiado** | Fluxo wizard de elaboração de PEI (Plano Educacional Especializado) em 9 etapas, baseado no modelo Jonas. O sistema pré-preenche capacidades, necessidades, objetivos e estratégias a partir da avaliação pedagógica e recomendações da equipe multidisciplinar (`peiRecomendado`). Disponível na ficha do aluno (Professor) quando há PEI recomendado pendente. |
